...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      // 1. Memberships → then load each project doc
      getDocs(query(collection(db, 'projectMembers'), where('userId', '==', userId)))
        .then(async snap => {
          const projectIds = snap.docs.map(d => d.data().projectId as string)
          if (!projectIds.length) return []
          const projectDocs = await Promise.all(projectIds.map(pid => getDoc(doc(db, 'projects', pid))))
          return projectDocs.filter(d => d.exists()).map(d => {
            const data = d.data()!
            return {
              id:          d.id,
              title:       data.title,
      ...
      getDocs(query(collection(db, 'contributions'), where('userId', '==', userId)))
        .then(snap => snap.docs
          .map(d => {
            const c = d.data()
            return {
              ...
              message:   c.message,
              type:      c.type,
              userName:  c.userName ?? userName ?? 'Unknown',
      ...
      // 3. Event registrations (denormalized — no extra fetch needed)
      ...
      <main className="max-w-5xl mx-auto px-6 pt-32 pb-24">
        ...
        <div className="grid grid-cols-3 gap-4 mb-12">
          {[
            ...
            { label: 'Contributions', value: dataLoading ? '—' : contributions.length },
